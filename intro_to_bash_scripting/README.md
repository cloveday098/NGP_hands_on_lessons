...
_README authored by Christian Goethert; Updated by Chance Loveday_
...
echo a + b   #a and b are not referenced
#a + b
echo $a + $b   #a and b are referenced, but not added
#14 + 2
echo "$((a + b))"  #a and b are referenced, encapuslated in arithmetic, and computed
#16
echo "$((a * b))"
#24
...
You can run several tasks simultaneously using the `&` operator. This is by no means HPC, but it allows to run multiple scripts in parallel rather than running them one at a time.

```
#!/bin/bash

# Runs task1 and task2 in parallel
./task1 &
./task2 &

wait     # Waits to continue until both tasks are finished
```
...
### cowsay
Bash has no shortage of whimsical commands to keep your scripting experience entertaining. Cowsay is what it sounds like: it works functionally like echo but displays all output inside a text bubble coming from a cow or various creatures and characters.

![Cow](./images/cowsay-1.png)

Assuming you are running a Bash shell on your local machine, install the cowsay package with ```npm install -g cowsay```. You may need permission to install packages if using a computing cluster, such as Odo.

| Flag | Mode                           |
|-------------|-------------------------|
| -b | Borg Mode --> cyborg cow         |
| -d | Dead Mode                        |
| -g | Greedy Mode                      |
| -p | Paranoia Mode                    |
| -t | Tired Mode                       |
...
Flags are one way to change your cow's appearance. There are also options for customomizing the cow's eyes and tongue.

Why stop simply at cows? Cowsay has dozens of creatures and characters to choose from. To view which ones you have installed, run ```cowsay -l```. We highlight some of these typeset images, including but not limited to the following:
- ![Dragon](./images/cowsay-dragon.png)
- ![Bees](./images/cowsay-bees.png)
- ![Ackbar](./images/cowsay-ackbar.png)
- ![Pterodactyl](./images/cowsay-pterodactyl.png)

You can specify which image you want to use via this syntax: ```cowsay -f <image name> "text"```.

While cowsay may not be the most functionally sound command, it is arguably one of the most entertaining to use.
